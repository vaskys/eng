...
bool abs_sdl::sdl_init(int screenW,int screenH,string title,bool resize,bool fullscreen)
{
  ...
  {
      debug_print("GLEW ERRROR WINDOWS");
  }
...
void abs_sdl::sdl_pool_events()
{
  ...
  //   Uint32 et = ct - startTime;
  //   if(et >= 1000) {
  //   float xd = frames/ (et / 1000.0f);
  //   cout<<xd<<endl;
  //   frames = 0;
  //   startTime = ct;
  // }
...
void abs_sdl::opengl_clear_color(float r,float g,float b,float a)
{
...
void abs_sdl::trap_mouse(SDL_bool v) {
  SDL_SetWindowGrab(window,v);
}

float abs_sdl::get_xd() {
  return 0;
}
